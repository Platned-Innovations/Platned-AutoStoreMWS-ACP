...
BEGIN
    ...
    BEGIN    
      ...
      FOR line_rec IN get_pl_lines_ LOOP
        ...
        c_value_ := c_value_ || ''{'' ||
            ...
            ''"ExtPickListId": "CO^'' || line_rec.PICK_LIST_NO || ''^",'' ||
            ''"ExtPickListLineId": '' || c_line_no_ || '','' ||
            ''"ExtOrderId": "CO^'' || line_rec.PICK_LIST_NO || ''^'' || line_rec.ORDER_NO || ''^'' || 
                          line_rec.LINE_NO || ''^'' || line_rec.REL_NO || ''^",'' ||
            ''"ExtOrderLineId": '' || c_line_no_ || '','' ||
            ''"ExtProductId": "'' || line_rec.PART_NO || ''",'' ||
            ''"BatchId": "'' || NVL(line_rec.LOT_BATCH_NO, '''') || ''",'' ||
            ''"Quantity": '' || line_rec.QTY_ASSIGNED || '','' ||
            ''"OrderTypeId": "CO",'' ||
            ''"OrderTypeText": "Customer Order",'' ||
      ...
      IF (c_count_ = 0) then
        ...
        CLIENT_SYS.Add_To_Attr(''CF$_C_PL_TYPE'', ''CO'', attr_cf_);
    ...
    Transaction_SYS.Deferred_Call('Fnd_Event_Action_API.Action_Executeonlinesql',
                                  ...
                                  Language_SYS.Translate_Constant('Event',
                                                                  'UPDATES: Platned AS Event for - Build Pick List Lines JSON. (CO)',
                                                                  NULL));
    sql_msg_ := null;
