...
BEGIN
    ...
    planned_ship_date_ VARCHAR2(50) := ''&C_PLANNED_SHIP_DATE'';
    planned_ship_time_ VARCHAR2(50) := ''&C_PLANNED_SHIP_TIME'';
    ...
    BEGIN    
      ...
      FOR line_rec IN get_pl_lines_ LOOP
        ...
        c_value_ := c_value_ || ''{'' ||
            ...
            ''"ExtPickListId": "SO^'' || line_rec.PICK_LIST_NO || ''^",'' ||
            ''"ExtPickListLineId": '' || c_line_no_ || '','' ||
            ''"ExtOrderId": "SO^'' || line_rec.PICK_LIST_NO || ''^'' || line_rec.ORDER_NO || ''^'' || 
                          line_rec.RELEASE_NO || ''^'' || line_rec.SEQUENCE_NO || ''^'' || line_rec.LINE_ITEM_NO || ''^",'' ||
            ''"ExtOrderLineId": '' || line_rec.PICK_LIST_NO || '','' ||
            ''"ExtProductId": "'' || line_rec.PART_NO || ''",'' ||
            ''"BatchId": "'' || NVL(line_rec.LOT_BATCH_NO, '''') || ''",'' ||
            ''"Quantity": '' || line_rec.QTY_ASSIGNED || '','' ||
            ''"OrderTypeId": "SO",'' ||
            ''"OrderTypeText": "Shop Order",'' ||
            ''"ExtPickDate": "'' || planned_ship_date_ || ''",'' ||
            ''"ExtPickTime": "'' || planned_ship_time_ || ''",'' ||
            ''"OrderLineNote": "'' || NVL(line_rec.CONTRACT, '''') || ''",'' ||
            ''"StockReservationKey": "'' || NVL(line_rec.CONTRACT, '''') || ''",'' ||
            ''"LineItemNo": "'' || c_line_no_ || ''"'' ||              
            ''}'';
      END LOOP;
      ...
      IF (c_count_ = 0) then
        ...
        CLIENT_SYS.Add_To_Attr(''CF$_C_PL_TYPE'', ''SO'', attr_cf_);
    ...
    Transaction_SYS.Deferred_Call('Fnd_Event_Action_API.Action_Executeonlinesql',
                                  ...
                                  Language_SYS.Translate_Constant('Event',
                                                                  'UPDATES: Platned AS Event for - Build Pick List Lines JSON. (SO)',
                                                                  NULL));
    sql_msg_ := null;
