...
BEGIN    
    ...
    planned_ship_date_ VARCHAR2(50) := ''&C_PLANNED_SHIP_DATE_PRECEIPT'';
    planned_ship_time_ VARCHAR2(50) := ''&C_PLANNED_SHIP_TIME_PRECEIPT'';
    ...
    BEGIN    
      ...
      FOR line_rec IN get_pl_lines_ LOOP      
        ...
        c_value_ := c_value_ || ''{'' ||
            ...
            ''"ExtPickListId": "PR^'' || line_rec.PICK_LIST_NO || ''^",'' ||
            ''"ExtPickListLineId": '' || c_line_no_ || '','' ||
            ''"ExtOrderId": "PR^'' || line_rec.PICK_LIST_NO || ''^'' || line_rec.SOURCE_REF1 || ''^'' || 
            ...
            ''"OrderTypeId": "PR",'' ||
            ''"OrderTypeText": "Purchase Receipt Return",'' ||
            ''"ExtPickDate": "'' || planned_ship_date_ || ''",'' ||
            ''"ExtPickTime": "'' || CASE WHEN planned_ship_time_ IS NULL OR planned_ship_time_ = ''0'' THEN ''000000'' 
      ...
      IF (c_count_ = 0) then
        ...
        CLIENT_SYS.Add_To_Attr(''CF$_C_PL_TYPE'', ''PR'', attr_cf_);
    ...
    Transaction_SYS.Deferred_Call('Fnd_Event_Action_API.Action_Executeonlinesql',
                                  ...
                                  Language_SYS.Translate_Constant('Event',
                                                                  'UPDATES: Platned AS Event for - Build Pick List Lines JSON. (PR)',
                                                                  NULL));
    sql_msg_ := null;
